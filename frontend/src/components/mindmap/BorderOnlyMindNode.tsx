/**
 * BorderOnlyMindNode - Clean border-only style node
 * No background, just border - minimal and clean
 */
...
export interface BorderOnlyNodeData {
  label: string;
  description?: string;
  status?: string;
  priority?: string;
...
function BorderOnlyMindNode({ data, id, selected }: NodeProps<BorderOnlyNodeData>) {
  const [isEditing, setIsEditing] = useState(false);
  const [label, setLabel] = useState(data.label);

  const borderColor = convertColor(data.color);
  ...
  return (
    <>
      ...
      {/* Node Content - Border Only, No Background */}
      <div
        style={{
          padding: '12px 16px',
...
export default memo(BorderOnlyMindNode);
