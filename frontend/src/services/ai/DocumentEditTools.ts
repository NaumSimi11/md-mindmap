...
export class DocumentEditTools {
   ...
   */
    ...
    /**
     * Smart helper: Create checkbox list with correct state
     */
    static formatCheckboxList(items: string[], checked: boolean = false): string {
        const checkMark = checked ? 'x' : ' ';
        return items.map(item => `- [${checkMark}] ${item}`).join('\n');
    }

    /**
     * Smart helper: Create bullet list with icons
     */
    static formatBulletList(items: Array<{ text: string; icon?: string }>): string {
    ...
    /**
     * Smart helper: Create numbered list
     */
    static formatNumberedList(items: string[]): string {
        return items.map((item, index) => `${index + 1}. ${item}`).join('\n');
    }

    /**
     * Smart helper: Create table
     */
        ...
        return [
            {
                ...
                parameters: {
                    ...
                    properties: {
                        ...
                        newContent: {
                            type: 'string',
                            description: 'New content (for replace/insert actions). Use correct markdown: checkboxes "- [ ]" or "- [x]", bullets "- ", numbered "1. "'
                        },
                        reason: {
                            type: 'string',
            ...
            },
            {
                ...
                parameters: {
                    ...
                    properties: {
                        ...
                        content: {
                            type: 'string',
                            description: 'Section content with proper markdown: "- [ ]" for unchecked, "- [x]" for checked, "- " for bullets, "1. " for numbered'
                        },
                        position: {
                            oneOf: [
